...
export const update = async (req, res, next) => {
    try {
        const { id, name, startTime, endTime, period } = req.body;

        if(!id) {
            const error = new Error('Không có thời gian để cập nhật.')
        ...
        const QWhere = { _id: id };
        const QUpdate = { $set: { name: name, startTime: startTime, endTime: endTime, period: period, timeDetail: TimeDetails.map(item => item._id) } };
        
        const result = await Time.updateOne(QWhere, QUpdate);
